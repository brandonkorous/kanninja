...
- **AI-native tools are off the roadmap.** kanNINJA removed its built-in AI
  entirely — there are no first-party reasoning models left in the product.
  `break_down_task`, `estimate_task`, `suggest_next`, `summarize_board`,
  `draft_standup`, and `extract_tasks` will not ship. The connected agent
  already does all six, with more context than we ever had. Clan tools remain
  planned.
...
- MCP tool annotations on every tool. The 12 read tools carry `readOnlyHint: true`; the 30 write tools carry `destructiveHint: true`. Hosts use these to apply correct auto-permissions, and they are required for the Anthropic Connectors Directory.
